...
@section('content')
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
        ...
        <x-stat-card
            ...
            <div class="mt-2 h-1 bg-[#f0e8dc] rounded-full overflow-hidden">
    ...
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="col-span-2 bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-[#f0e8dc]">
            ...
            @else
                <div class="divide-y divide-[#f5f0e8]">
                    @foreach($arrivalsToday as $booking)
                        <div class="flex items-center gap-4 px-5 py-3 hover:bg-[#faf7f2] transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-[#a09080]">Chambre {{ $booking->room->number }} — {{ $booking->room->roomType->name }}</p>
                            </div>
                            <p class="text-xs text-[#a09080] flex-shrink-0">{{ $booking->adults_count }} pers.</p>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="px-5 py-4 space-y-3">
                @php
                    $statusRows = [
                        ['label' => 'Disponibles',  'count' => $stats['rooms_available'],   'color' => 'bg-emerald-400'],
                        ['label' => 'Occupées',      'count' => $stats['rooms_occupied'],    'color' => 'bg-blue-400'],
                        ['label' => 'En nettoyage',  'count' => $stats['rooms_cleaning'],    'color' => 'bg-yellow-400'],
                        ['label' => 'Maintenance',   'count' => $stats['rooms_maintenance'], 'color' => 'bg-orange-400'],
                    ];
                @endphp
                @foreach($statusRows as $row)
                    <div class="flex items-center gap-3">
                        <span class="w-2 h-2 rounded-full flex-shrink-0 {{ $row['color'] }}"></span>
                        <span class="flex-1 text-xs text-[#6a5a4a]">{{ $row['label'] }}</span>
                        <div class="w-20 h-1 bg-[#f0e8dc] rounded-full overflow-hidden">
                            @if($stats['rooms_total'] > 0)
                                <div class="h-full {{ $row['color'] }} rounded-full"
                ...
                @endforeach
                <div class="pt-3 border-t border-[#f0e8dc] flex justify-between items-center">
                    <span class="text-xs text-[#a09080]">Total chambres</span>
